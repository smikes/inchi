...
(function () {
    ...
    var inchi = exports,
        ...
        inner_GetInChIFromMolecule = inchilib.GetInChIFromMolecule,
    ...
    inchi.inchilib.GetINCHI = function GetINCHI_wrap(struct, callback) {
        // TODO(SOM) check for input validity
        var mol = { atoms: [], bonds: [], stereo0D: [], options: '' };

        console.log(JSON.stringify(struct));

        struct.atom.forEach(function (a, i) {
            mol.atoms.push({elname: a.elname});
            a.neighbor.forEach(function (b) {
                mol.bonds.push({from: i, to: b, order: 1, stereo: 0});
            });
        });

        inner_GetInChIFromMolecule(mol, function (err, result) {
    ...
    inchi.inchilib.GetInChIFromMolecule = function GetInChIFromMolecule_wrap(mol, callback) {
        // TODO(SOM) check for input validity
